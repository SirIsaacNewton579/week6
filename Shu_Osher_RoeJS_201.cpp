...
int main(){
    ...
    double rho,u,p;
    int Nx = 201;
    double dx = 2.0*L/(Nx-1),dt = 0.001;
    double t_end = 0.1,Nt = round(t_end/dt);
    cout << "Nt=" << Nt << endl;
    double U[3][Nx];
    ...
    updateU(U[0],Nx,dx,dt,Nt,WENO5JS,5,2);  //FDS Roe格式 使用Weno格式，5个基架点，第一个点为j-2

    //输出
    ofstream csvfile;
    csvfile.open("Shu_Osher_Nx=201_Roe-weno5JS_t=0.1.csv", ios::out | ios::trunc);
