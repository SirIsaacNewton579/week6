...
int main(){
    ...
    double rho,u,p;
    int Nx = 401;
    double dx = 2.0*L/(Nx-1),dt = 0.0005;
    ...
    updateU(U[0],Nx,dx,dt,Nt,WENO5M,5,2);  //FDS Roe格式 使用Weno格式，5个基架点，第一个点为j-2

    //输出
    ofstream csvfile;
    csvfile.open("Shu_Osher_Nx=401_Roe-weno5M_t=0.1.csv", ios::out | ios::trunc);
